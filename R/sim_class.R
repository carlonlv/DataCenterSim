...
#' @keywords internal
check_valid.sim <- function(object) {
  model_choices <- c("AR1", "AR1_Markov", "Markov", "VAR1")
  ...
  errors <- character()
  if (length(object@name) != 1 | is.na(object@name) | all(object@name != model_choices)) {
    msg <- paste0("name must be one of ", paste(model_choices, collapse = " "), ".")
    errors <- c(errors, msg)
  }
...
#'
#' @slot name A character that can either be \code{"AR1"}, \code{"Markov"}, \code{"AR1_Markov"}, \code{"VAR1"}, additional slots may need to be supplied for different simulations.

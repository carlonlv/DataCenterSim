...
plot_heatmap_correlations <- function(dataset1, dataset2=NULL, window_size1, window_size2=NULL, response1, response2=NULL, corr_method, cores, name, ...) {
  ...
  save_path <- write_location_check(file_name = file_name, ...)
  plt <- lattice::levelplot(stats::as.formula("corr ~ window_size_x * window_size_y"), col.regions = grDevices::heat.colors(100, rev = TRUE), data = result_df, xlab = "Window Sizes of Past Observations", ylab = "Window Sizes of Current Observations")
